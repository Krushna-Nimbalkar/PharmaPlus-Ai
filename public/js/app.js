...
const renderMr = async () => {
  ...
  els.mrSection.innerHTML = `
    <h2>MR Dashboard</h2>
    <p class="hint">Use the form below to create a PR (Drug Proposal) and notify matching doctors.</p>
    <form id="drugForm" class="form-grid">
      ...
      <button type="submit">Create PR & Notify Doctors</button>
    </form>
  ...
  document.getElementById('drugForm').addEventListener('submit', async (e) => {
    ...
    const form = Object.fromEntries(new FormData(e.target));
    try {
      const data = await api('/api/mr/pr', 'POST', form);
